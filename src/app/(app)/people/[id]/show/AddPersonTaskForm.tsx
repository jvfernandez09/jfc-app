...
import {
  createTaskForPersonAction,
  type CreatePersonTaskState,
...
const initialState: CreatePersonTaskState = {
  success: false,
  message: "",
  errors: {},
};

export default function AddPersonTaskForm({ personId }: Props) {
  const formRef = useRef<HTMLFormElement | null>(null);

  const boundAction = createTaskForPersonAction.bind(null, personId);
  ...
  return (
    <div>
      ...
      {state.success && state.message && (
        <div className="mb-4">
          <AlertMessage variant="success" title="Success!" message={state.message} />
        </div>
      )}
